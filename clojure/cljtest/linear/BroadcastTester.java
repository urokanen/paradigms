...
public class BroadcastTester extends LinearTester {
    public static final List<Item.Fun> BROADCAST = Item.functions("b");
    ...
    protected void test(final int args) {
        ...
        for (int complexity = 0; complexity <= 20 / TestCounter.DENOMINATOR2; complexity++) {
            ...
            final List<Supplier<Item>> slices = slice(0, dims).map(Item::generator).toList();
    ...
    private void test(final int[] dims, final Stream<Supplier<Item>> generators) {
        final List<Item> args = generators.map(Supplier::get).map(item -> item.refill(random())).toList();
        ...
        final List<Item> fakeArgs = args.stream().map(arg -> extend(0, arg, newDims)).toList();
