...
//            System.out.format("Args=%d, complexity=%d%n", args, complexity);
    ...
    /// 4D Cuboid
    private static final List<Item.Fun> CUBOID_4 = Item.functions("c4");

    private static void cuboid4(final Test test) {
        for (int complexity = 1; complexity < 10; complexity++) {
            for (int size1 = 1; size1 < complexity; size1++) {
                for (int size2 = 1; size1 + size2 < complexity; size2++) {
                    for (int size3 = 1; size1 + size2 + size3 < complexity; size3++) {
                        test.test(Item.generator(size1, size2, size3, complexity - size1 - size2 - size3));
                    }
                }
            }
        }
    ...
    public static final Selector SELECTOR = new Selector(LinearTester.class, "easy", "hard")
            ...
            .variant("Cuboid4", variant(CUBOID_4, LinearTest::cuboid4))
