...
public class PrimesTester extends PrologTest {
    ...
    public void test() {
        ...
        Asserts.assertTrue("Time limit exceeded: %sms instead of %sms".formatted(time, limit), time <= limit);
    }

    public void runTests() {
        PrologUtil.measure(counter, "Init", () -> counter.println("Result: " + test(INIT, max)));
    ...
    void checkDivisors(final Rule rule, final boolean reversible, final Function<IntStream, IntStream> solution) {
        checkDivisorsG(rule, reversible, divisors -> solution.apply(divisors).boxed().toList());
    }
    ...
    protected void testBinary(final String name, final LongBinaryOperator op) {
        PrologUtil.measure(counter, name, () -> {
